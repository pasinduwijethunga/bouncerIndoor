    ...
    if ($result1) {
        for ($x = 0; $x < count($data['proObj']); $x++) {
            ...
            if (!$result2) {
                ...
                $sql = mysqli_query($connection,"SELECT * FROM batch_tbl WHERE pro_id = '$proid'");
                $row = mysqli_fetch_array($sql);
                array_push($stack,$row);
				
                if (count($row) == 0) {

                    $query3 = "INSERT INTO batch_tbl(`batch_id`,`pro_id`,`batch_no`,`batch_date`,`ex_date`,`cost`,`price`,`user_id`,`added_date`)VALUES(
                        '',
                        '$proid',
                        '$newBatch',
                        '$date',
                    ...
                    $sql1 = mysqli_query($connection,"SELECT * FROM batch_tbl WHERE pro_id = '$proid'");
                    ...
                    for ($y = 0; $y < count($productArr); $y++) {
                        ...
                        if ($productArr[$y]['price'] == $proprice){
                            ...
                            $query5 = "UPDATE stock_tbl SET stock_qty='$newStock'
                                WHERE 
                                batch_id='$batchId'";
